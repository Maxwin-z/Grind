...
final class AppDataSharingPreferences: ObservableObject {
  ...
  func toggleSelection(for app: MacApp) {
    ...
    setAppSelected(
      bundleId: app.bundleIdentifier, appName: app.name, isSelected: !currentlySelected)
  }
  ...
  func setApps(_ apps: [MacApp], isSelected: Bool) {
    ...
    for app in apps {
      if applySelectionChange(
        bundleId: app.bundleIdentifier, appName: app.name, isSelected: isSelected)
      {
        didChange = true
    ...
    if didChange {
      let action = isSelected ? "✅ Selected" : "❌ Deselected"
      print(
        "\(action) \(changedApps.count) apps: \(changedApps.prefix(5).joined(separator: ", "))\(changedApps.count > 5 ? "..." : "")"
      )
      persistSelections()
      notifySelectionChanged()
    }
  ...
  func updateKnownApps(_ apps: [MacApp]) {
    ...
    for app in apps {
      ...
      let colorHex = AppColorGenerator.colorHex(
        forBundleIdentifier: app.bundleIdentifier, appName: app.name)
      let metadata = AppSelectionMetadata(
        appName: app.name, iconData: iconData, accentColorHex: colorHex)
...
extension Notification.Name {
  static let appDataSharingSelectionChanged = Notification.Name(
    "AppDataSharingPreferences.selectionChanged")
}
...
struct AppColorGenerator {
  ...
  private static func stableColorIndex(for seed: String) -> Int {
    ...
    // FNV-1a hash
    var fnvHash: UInt64 = 14_695_981_039_346_656_037  // FNV offset basis
    for byte in seed.utf8 {
      fnvHash ^= UInt64(byte)
      fnvHash = fnvHash &* 1_099_511_628_211  // FNV prime
    }
    ...
    let mixedHash = (hash ^ (hash >> 16)) &* 0x85eb_ca6b
    let finalHash = (mixedHash ^ (mixedHash >> 13)) &* 0xc2b2_ae35
...
extension NSImage {
  ...
  func scaledPngData(maxDimension: CGFloat) -> Data? {
    ...
    draw(
      in: drawRect, from: NSRect(origin: .zero, size: size), operation: .sourceOver, fraction: 1.0)
